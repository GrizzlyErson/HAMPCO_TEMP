...
switch ($action) {
    ...
    case 'mark-all-read':
        try {
            ...
            if (!$stmt_approvals) throw new Exception("Prepare failed for approvals: " . $db->conn->error);
            ...
            $stmt_find = $db->conn->prepare($sql_find_messages);
            if (!$stmt_find) throw new Exception("Prepare failed for finding messages: " . $db->conn->error);
            ...
            if (!empty($unread_message_ids)) {
                ...
                $stmt_mark = $db->conn->prepare($sql_mark_messages);
                if (!$stmt_mark) throw new Exception("Prepare failed for marking messages: " . $db->conn->error);
