...
$(document).ready(function() {
    // Enhanced search functionality - searches both tables
    $("#searchInput").on("keyup", function() {
        var searchValue = $(this).val().toLowerCase().trim();
        
        // Search in New Members table
        $("#newMemberTable tbody tr").each(function() {
        ...
        // Search in Verified Members table
        $("#verifiedMemberTable tbody tr").each(function() {
            var rowText = $(this).text().toLowerCase();
            if (searchValue === '' || rowText.indexOf(searchValue) > -1) {
                $(this).show();
            } else {
    ...
    $(document).on('click', '.declineBtn, .removeBtn', function() {
        if ($(this).hasClass('cursor-not-allowed')) {
            return;
        }
        userId = $(this).data('id');
